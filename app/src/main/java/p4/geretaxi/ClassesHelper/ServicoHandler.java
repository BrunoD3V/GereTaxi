...
public class ServicoHandler {
    ...
    public Double getDistance(List<LatLng> latLngs) {
        ...
        //percorre a lista de pontos, calculando a distância entre cada um deles e somando esse valor `*a distancia total
    ...
    public List<LatLng> mergeCapture(List<LatLng> capturedLocations) {
        ...
        double dis = getDistance(mCapturedLocations);  //obtem a distância percorrida durante o serviço
        ...
        routes = getDirections(origin, destination);  //obtem o trajecto entre a praça de taxis e o inicio do serviço.
        if(distance == 0) {
            return mCapturedLocations;
        }
        dis += distance;
        mCapturedLocations= ListUtils.union(routes,mCapturedLocations); //concatena as Listas

        routes = null;
        routes = getDirections(termino, origin); //obtem o trajecto entre o final do serviço e a praça de taxi.
        dis += distance;
        distance = dis;

        mCapturedLocations= ListUtils.union(mCapturedLocations, routes); //concatena as Listas
    ...
    public List<LatLng> getRoute(List<LatLng> capturedLocations, GeoApiContext context) throws Exception {
        ...
        mCapturedLocations = capturedLocations;
        mSnappedPoints = snapToRoads(context); //faz a interpolação dos pontos para a estrada mais proxima
    ...
    private List<LatLng> getVisitedPlaces() {
        ...
        //percorre a lista de pontos e adiciona à nova lista apenas os que têem placeId diferente
