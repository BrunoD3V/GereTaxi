package p4.geretaxi.Constantes;

public class Constants {
    //TIPOS DE SERVICO
    public static final String ACIDENTE = "Acidente de Trabalho";
    public static final String VIAGEM = " Assistência em Viagem";
    public static final String PARTICULAR = "Serviço Particular";

    //TOSTRING() SERVICO
    public static final String PROCESSO_TOSTRING = "Processo: ";
    public static final String CLIENTE_TOSTRING = "Cliente: ";
    public static final String TIPO_SERVICO_TOSTRING = "Tipo: ";
    public static final String HORA_DE_INICIO_TOSTRING = "Hora de Inicio: ";
    public static final String DATA_TOSTRING = "Data: ";
    public static final String ORIGEM_TOSTRING = "Local de carga: ";
    public static final String DESTINO_TOSTRING = "Local de descarga: ";
    public static final String TRAJETO_TOSTRING = "Trajecto: ";
    public static final String DISTANCIA_TOSTRING = "Distância percorrida: ";
    public static final String HORAS_DE_ESPERA_TOSTRING = "Horas de espera: ";
    public static final String NUM_PASSAGEIROS_TOSTRING = "Número de passageiros: ";
    public static final String CUSTO_PORTAGENS_TOSTRING = "Custo portagens: ";
    public static final String KMS_TOSTRING = " kms";

    //TOSTRING() CLIENTE
    public static final String MORADA_TOSTRING = "Morada: ";
    public static final String CODIGO_POSTAL_TOSTRING = "Codigo Postal: ";
    public static final String CONTACTO_TOSTRING = "Contacto: ";
    public static final String EMAIL_TOSTRING = "Email: ";
    public static final String TIPO_CLIENTE_TOSTRING = "Tipo de Cliente: ";
    public static final String NIF_TOSTRING = "NIF: ";

    //XML TAGS
    public static final String PONTO_XML = ".xml";
    public static final String CLIENTE_XML = "cliente";
    public static final String ID_XML = "id";
    public static final String MORADA_XML = "morada";
    public static final String CODIGO_POSTAL_XML = "codigo-postal";
    public static final String NOME_XML = "nome";
    public static final String NIF_XML = "nif";
    public static final String CONTACTO_XML = "contacto";
    public static final String MAIL_XML = "email";
    public static final String TIPO_XML = "tipo";
    public static final String TIPO_SERVICO_XML = "tipo-servico";
    public static final String NOME_CLIENTE_XML = "nome-cliente";
    public static final String HORA_DE_INICIO_XML = "hora-de-inicio";
    public static final String TRAJETO_SOAP_XML = "trajeto";
    public static final String NUM_PASSAGEIROS_XML = "numero-passageiros";
    public static final String CUSTO_PORTAGENS_XML = "custo-portagens";
    public static final String ORIGEM_XML = "origem";
    public static final String DESTINO_XML = "destino";
    public static final String TRAJETO_XML = "trajeto";
    public static final String DISTANCIA_XML = "distancia";
    public static final String HORASDEESPERA_XML = "horas-de-espera";
    public static final String DATA_XML = "data";
    public static final String PROCESSO_XML = "processo";
    public static final String SERVICO_XML = "servico";

    //MENSAGENS
    public static final String N_PASSAGEIROS_VALIDO = "Insira um número válido de passageiros";
    public static final String N_VALIDO = "Insira um número válido";
    public static final String N_POSITIVO = "O valor tem que ser positivo";
    public static final String REGISTO_EXISTE = "Esse utilizador já existe";

    //COORDENADAS
    public static final String WPT = "wpt";
    public static final String LAT = "lat";
    public static final String LON = "lon";

    //GERAIS
    public static final String ID_MOTORISTA = "ID_MOTORISTA";
    public static final String CLIENTES = "clientes";
    public static final  String TRAJETO = "Trajecto";
    public static final String LOG = "Algo";
    public static final String KEY = "SLB";
    public static final String SALT = "Minas";
    public static final String REGISTO_SEM_NET= "no net";

    //SESSOES E LOGINS
    public static final String EMAIL = "Email";
    public static final String PASS = "Pass";
    public static final String SESSION = "session";
    public static final String VALIDADE = "validade";

    //BOOLEANS
    public static final String TRUE = "verdadeiro";
    public static final String FALSE = "falso";

    //IP
    public static final String IP = "192.168.1.5";

    //OUTROS
    public static final String INTENT_SERVICO = "servico";
    public static final String STRING_VAZIA = "";
}
